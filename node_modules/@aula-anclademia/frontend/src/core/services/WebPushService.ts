...
export default class WebPushService {
  ...
  private static async getVapidPublicKey() {
    try {
      ...
      return "BOBMHp1S5eut8OUWt4qU5_hpR-_FytqcuPHEnOZrPO6MKRz3mw_Gp-QOQ9TdgQ6LSKaKS4P99qcvT49aSv9o120";
    } catch (e) {
  ...
  public static async getBrowserSubscription(): Promise<PushSubscription | null> {
    console.log('----AuthService.lastState.value',AuthService.lastState.value);
...
console.log('----subscription111',subscription);
      if (!subscription) {
        ...
        // });
        console.log('----subscription222',subscription);
      }
  ...
  public static async tryUpdateSubscription({
    ...
    try {
      if (subscription) {
        console.log('----subscriptionqqqqqqqqqqqqqqqqqqqqqqqqqqq');
